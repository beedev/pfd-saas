...
/**
 * Transformation tracker — daily checklist.
 *
 * Single-page editor: pick a date, tick checkboxes, fill text fields, log
 * weight + journal, hit "Save day". Edit mode reveals item / section /
 * plan-level controls. Ported verbatim from personal-v1 (only the @dxp/ui
 * imports and a few prop names differ).
 */
...
export default function TransformationPage() {
  ...
  // Progress counts only check-kind items.
  ...
  const estimateItem = async (item: Item, text: string) => {
    ...
    try {
      ...
      const j = await r.json();
      // Stub mode → calories null; don't set a nutrition entry.
      if (j.estimate && j.estimate.calories != null) {
        setNutrition((prev) => ({
          ...prev,
          [item.id]: { calories: j.estimate.calories, proteinG: j.estimate.proteinG ?? 0 },
        }));
      }
    } catch (e) {
