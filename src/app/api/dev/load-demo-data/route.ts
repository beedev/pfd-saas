 ...
 *
 * Sprint 6.1.6 — FULL version. This is a port of
 * `scripts/seed-demo-bxdeva.mjs` (which is hard-coded to the BXDEva
 * dev user) adapted for any authenticated session. It seeds ~23 tables
 * with FY 2025-26 data that drives:
 *   - positive net worth (~₹2–3Cr)
 *   - all 9 asset classes non-zero
 *   - tax dashboard exercise every Sec 80 + capital-gain bucket
 *   - retirement / projections / cashflows screens with real inputs
 *
 * Every row that supports it carries a `notes` value starting
 * `DEMO-SEED:` so the wipe endpoint (and this endpoint's own cleanup
 * pass) can find them. Re-running is idempotent: we DELETE all
 * DEMO-SEED rows for this user first, then INSERT fresh.
 *
 * The whole seed runs inside a single Drizzle transaction — either it
 * all lands or none of it does. No partial writes on failure.
 */
...
export async function POST() {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      // Tables with a `notes` column use the LIKE-marker delete. Child
      // tables that cascade from a parent (health_insurance_cards from
      // health_insurance_policies, vehicle_* from vehicles) are wiped
      // automatically when the parent goes. budget_entries cascades
      // from budget_categories, so we delete the categories below.
      const NOTES_TABLES = [
        salaryIncome,
        otherSourcesIncome,
        rentalHistory, // child of real_estate; FK cascade fires when real_estate goes
        realEstate,
        capitalGains,
        taxDeductions,
        tdsCredits,
        sips, // child of mutual_funds; FK cascade fires when mutual_funds goes
        holdings,
        mutualFunds,
        goldHoldings,
        npsAccounts,
        ...
        liabilities,
        insurancePolicies,
        healthInsurancePolicies, // children cascade
        vehicles, // children cascade
        subscriptions,
      ] as const;
      ...
      // budget_categories cascades to budget_entries on delete. We
      // identify our seeded categories by the same sort_order >= 9000
      // marker the BXDEva script uses, so prior human-curated categories
      // are untouched.
      await tx
      ...
      // itr_form_selection: one row per (user, fy), no notes column.
      await tx
      ...
      // financial_goals: no notes column. Delete by the distinctive
      // demo goal names. Real user goals won't share these strings.
      await tx
      ...
      // The row should already exist (created during onboarding). We
      // only flip the tax-setup booleans so deductions compute correctly.
      // If the user is brand new and skipped onboarding, this is a no-op.
      await tx
      ...
      await tx.insert(salaryIncome).values({
        ...
        exemptionsPaisa: 0,
        section16Paisa: 5_000_000, // ₹50k standard deduction in paisa
      ...
      // 3. Land plot — drives 80EEA computation (first-home, post-Apr-2019
      // disbursal, stamp value ≤ ₹45L, carpet ≤ 968 sqft).
      ...
      // ─── rental_history — 3 prior FYs on Whitefield ───────────────
      const history = [
      ...
      // ─── capital_gains — 4 rows across every bracket ──────────────
      const capRows = [
        {
      ...
      // ─── tax_deductions — every Sec 80 bucket ─────────────────────
      type DedRow = {
        sec: string;
        desc: string;
        amount: number;
      ...
      const dedRows: DedRow[] = [
        // 80C — total ~₹3L, app-side capped at ₹1.5L
        ...
        // 80CCD(2) — employer NPS (eligible under NEW)
      ...
      // ─── mutual_funds — 8 across categories ───────────────────────
      // fund_type = AMFI scheme class; category = rate-bucket key (Sprint 5.7).
      // LIQUID maps to DEBT for the rate bucket.
      const funds = [
      ...
      // ─── nps_accounts — 1 Tier-I ──────────────────────────────────
      {
        ...
        await tx.insert(npsAccounts).values({
          ...
          accountHolder: session.user.name ?? 'Demo User',
          pan: 'XXXXX1234X',
          tier: 'TIER1',
          status: 'ACTIVE',
      ...
      // ─── small_savings_accounts — PPF, SSY, NSC ───────────────────
      type SSRow = {
        scheme: 'PPF' | 'SSY' | 'NSC';
        acct: string;
        holder: string;
      ...
      const ssRows: SSRow[] = [
        {
          scheme: 'PPF', acct: 'PPF8001100200300', holder: 'Demo User',
        ...
        {
          scheme: 'NSC', acct: 'NSC8009988776655', holder: 'Demo User',
          institution: 'India Post Indiranagar',
      ...
      for (const f of fds) {
        const periods = f.tenure / 3; // quarterly compounding
      ...
      // ─── liabilities — home loan + credit card ────────────────────
      ...
      // ─── health_insurance — 1 family floater + 4 cards ────────────
      ...
      const fam = [
        { name: session.user.name ?? 'Demo User', rel: 'SELF' as const, dob: '1988-04-12', gender: 'M' },
      ...
      // ─── vehicle + insurance + PUC + service log ──────────────────
      ...
      // ─── subscriptions — 6 across categories ──────────────────────
      const subs = [
      ...
      // ─── budget_categories + budget_entries (2 cats × 12 mo) ──────
      ...
      for (let i = 0; i < monthsFY.length; i++) {
        ...
        // Deterministic ±20% drift seeded from month index.
      ...
      // ─── itr_form_selection — ITR-2 wizard answer ─────────────────
      const wizardAnswers = {
        hasSalary: true,
