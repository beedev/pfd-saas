...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = fy
      ? await db.select().from(salaryIncome).where(and(eq(salaryIncome.financialYear, fy), eq(salaryIncome.userId, userId))).orderBy(desc(salaryIncome.id))
      : await db.select().from(salaryIncome).where(eq(salaryIncome.userId, userId)).orderBy(desc(salaryIncome.id));
