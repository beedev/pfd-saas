...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const existing = await db.select().from(tdsCredits).where(and(eq(tdsCredits.id, numericId), eq(tdsCredits.userId, userId))).limit(1);
    ...
    const result = await db.update(tdsCredits).set(update).where(and(eq(tdsCredits.id, numericId), eq(tdsCredits.userId, userId))).returning();
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    await db.delete(tdsCredits).where(and(eq(tdsCredits.id, numericId), eq(tdsCredits.userId, userId)));
