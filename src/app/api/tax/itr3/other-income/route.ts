...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = fy
      ? await db.select().from(otherSourcesIncome).where(and(eq(otherSourcesIncome.financialYear, fy), eq(otherSourcesIncome.userId, userId))).orderBy(desc(otherSourcesIncome.id))
      : await db.select().from(otherSourcesIncome).where(eq(otherSourcesIncome.userId, userId)).orderBy(desc(otherSourcesIncome.id));
