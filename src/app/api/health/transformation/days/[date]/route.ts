...
// Returns the day row (or an empty stub if not yet saved) plus the checks map.
export async function GET(_request: NextRequest, { params }: Params) {
  try {
    ...
    const dayRows = await db
      ...
      .where(
        and(eq(transformationDays.planId, plan.id), eq(transformationDays.date, date)),
      )
      .limit(1);
    ...
    const checkRows = await db
      ...
      .where(eq(transformationChecks.dayId, stored.id));
    const checks: Record<number, boolean> = {};
    const texts: Record<number, string> = {};
    const nutrition: Record<
      number,
      { calories: number; proteinG: number; estimatedAt: number | null }
    > = {};
    for (const c of checkRows) {
      checks[c.itemId] = c.checked === 1;
    ...
    console.error('GET day failed:', err);
    return NextResponse.json({ error: 'Failed to load day' }, { status: 500 });
  }
}

// PUT /api/health/transformation/days/[date]
// Upsert the day row plus its checks.
// Body: { currentWeightKg?, journal?, checks?: { [itemId]: boolean } }
  ...
  try {
    ...
    if (touchedIds.size) {
      ...
      for (const itemId of itemIds) {
        ...
        const checkVal =
          key in incomingChecks ? (incomingChecks[key] ? 1 : 0) : null;
        ...
        if (existing) {
          ...
          if (checkVal != null && existing.checked !== checkVal) update.checked = checkVal;
          if (textVal !== undefined && existing.textValue !== textVal) {
          ...
          if (Object.keys(update).length > 0) {
            await db
              ...
              .set(update)
              .where(eq(transformationChecks.id, existing.id));
          }
        } else {
    ...
    const day = (
      await db.select().from(transformationDays).where(eq(transformationDays.id, dayId))
    )[0];
    const checkRows = await db
      .select()
      .from(transformationChecks)
      .where(eq(transformationChecks.dayId, dayId));
    const checks: Record<number, boolean> = {};
    const texts: Record<number, string> = {};
    for (const c of checkRows) {
      checks[c.itemId] = c.checked === 1;
    ...
    console.error('PUT day failed:', err);
