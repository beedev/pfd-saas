...
/**
 * Variant of guardSection for sections whose "already done" check
 * needs a custom predicate (e.g. column-value-based, not just "table
 * has rows for this user"). `isDoneFn` returns a Promise<boolean>:
 * true means skip, false means run.
 */
async function guardSectionRaw(name, isDoneFn, fn) {
  if (await isDoneFn()) {
    sectionsSkipped.push(name);
    return;
  }
...
// ─── financial_goals_extended (Sprint 3.5 Phase 3) ────────────────
// Upgrades existing demo goals with realistic disbursement shapes and
// inserts a few new ones to exercise every disbursement_type. Also
// wires savings_asset_inclusion so the funded% calculation has data.
// Guard checks for an EMERGENCY-type row — present means we've already
// upgraded this user.
await guardSectionRaw('goals_extended', async () => {
  const existingMarker = await sql`
    SELECT id FROM financial_goals
    WHERE user_id = ${userId} AND goal_type = 'EMERGENCY'
    LIMIT 1
  `;
  return existingMarker.length > 0;
}, async () => {
  // 1. Upgrade existing demo goals
  await sql`
    UPDATE financial_goals
    SET goal_type = 'TRAVEL',
        disbursement_type = 'LUMPSUM',
        expected_return_pct = 8,
        inflation_pct = 6
    WHERE user_id = ${userId} AND name = 'Vacation Europe'
  `;
  await sql`
    UPDATE financial_goals
    SET goal_type = 'EDUCATION',
        disbursement_type = 'INFLATION_SWP',
        disbursement_amount_per_yr_paisa = ${lakh(5)},
        disbursement_years = 4,
        disbursement_start_date = '2035-06-01',
        growth_pct_per_yr = 8,
        expected_return_pct = 11,
        inflation_pct = 8,
        target_amount = ${lakh(20)}
    WHERE user_id = ${userId} AND name = 'Daughter College Fund'
  `;

  // 2. New goals — one HOUSE (LUMPSUM), one EMERGENCY (LUMPSUM)
  await sql`
    ...
    VALUES (${userId}, 'House upgrade', ${lakh(50)},
            '2030-12-31', 0, '#f59e0b', true,
            'HOUSE', 'LUMPSUM', 10, 6)
  `;
  await sql`
    INSERT INTO financial_goals
      (user_id, name, target_amount, target_date, current_amount,
       color, is_active, goal_type, disbursement_type,
       expected_return_pct, inflation_pct)
    VALUES (${userId}, 'Emergency fund', ${lakh(10)},
            '2027-12-31', 0, '#ef4444', true,
            'EMERGENCY', 'LUMPSUM', 6, 6)
  `;

  // 3. Asset mapping rows. Goals must be looked up by name since IDs
  // are autogenerated.
  const goalIdByName = new Map();
  const allGoals = await sql`
    SELECT id, name FROM financial_goals WHERE user_id = ${userId}
  `;
  for (const g of allGoals) goalIdByName.set(g.name, g.id);

  const college = goalIdByName.get('Daughter College Fund');
  const house = goalIdByName.get('House upgrade');
  const emergency = goalIdByName.get('Emergency fund');

  // College → Mutual Funds (aggregate)
  if (college) {
    await sql`
      INSERT INTO savings_asset_inclusion
        (user_id, asset_class, source_id, included, goal_id)
      VALUES (${userId}, 'MUTUAL_FUNDS', NULL, true, ${college})
    `;
  }
  // House → Stocks + Mutual Funds (aggregate)
  if (house) {
    await sql`
      INSERT INTO savings_asset_inclusion
        (user_id, asset_class, source_id, included, goal_id)
      VALUES (${userId}, 'STOCKS', NULL, true, ${house})
    `;
    await sql`
      INSERT INTO savings_asset_inclusion
        (user_id, asset_class, source_id, included, goal_id)
      VALUES (${userId}, 'MUTUAL_FUNDS', NULL, true, ${house})
    `;
  }
  // Emergency → all active FDs (itemized — one row per FD)
  if (emergency) {
    const fdRows = await sql`
      SELECT id FROM fixed_deposits
    ...
    for (const fd of fdRows) {
      await sql`
        INSERT INTO savings_asset_inclusion
          (user_id, asset_class, source_id, included, goal_id)
        VALUES (${userId}, 'FIXED_DEPOSITS', ${fd.id}, true, ${emergency})
      `;
    }
  }
});
