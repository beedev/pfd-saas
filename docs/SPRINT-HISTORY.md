# Sprint history — pfd-saas (Artha)

> Moved out of `CLAUDE.md` on 2026-09-12. It was 852 of that file's 1360 lines
> (63%), and `CLAUDE.md` loads into **every** session's context — so 850 lines of
> completed-work narrative were a standing cost on every window while helping
> nobody act today.
>
> **Nothing was deleted.** This is the verbatim record. `CLAUDE.md` keeps the
> current state, a headline list of what shipped, and the deferred / still-open
> items, because those are decision-relevant.
>
> Day-by-day detail for recent work lives in `docs/changes/CHANGES-YYYY-MM-DD.md`.
> Architecture is `docs/ARCHITECTURE.md`. Operations is
